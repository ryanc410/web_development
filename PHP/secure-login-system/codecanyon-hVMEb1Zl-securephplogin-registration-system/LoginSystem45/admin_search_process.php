...
$role_id = mysqli_real_escape_string($con, $_POST["role_description"]);
//prevent xss
...
$query = "SELECT username, case when activ_status='0' then 'Not Activated' when activ_status='1' then 'activated' end as activ_status,  'email' as source FROM " . $table_name . " where email='" . $email . "' or username='" . $username . "' or role=" . $role_id . " UNION ALL SELECT username,  'activated', source FROM " . $table_name_social . " where email='" . $email . "' or username='" . $username . "' or role=" . $role_id;
